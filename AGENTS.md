...
Loop Control Tower: cada slice vive en su issue de GitHub, se despacha con
`/ct-next` a un worktree en `.worktrees/<n>` (rama `feat/<n>`) y termina en un
PR contra `main`. La sesión del checkout principal coordina (groomea, despacha,
revisa, mergea); las de los worktrees implementan y paran. Ver contrato §9 más
abajo.

## Commit & PR rules
- Rama por defecto: `main`. PRs contra `main` salvo que el dispatch diga otra base.
- Cada PR de slice lleva `Closes #N` en el cuerpo (N = número de issue).
