...
import { createRepoFixture, headShaOf } from '../testing/repo-fixture.js'
...
test('the heat carries the HEAD sha, null without commits', async () => {
  ...
  // not be hardcoded.
  const realHeadSha = headShaOf(fixture.path)
...
test('children sort by commits, then name', async () => {
  ...
  //
  // 'Widgets.ts' and 'app.ts' are also tied at 2 commits, and pin the
  // tie-break to code-unit order specifically: `localeCompare`'s locale
  // collation sorts 'app.ts' before 'Widgets.ts' (letters are compared before
  // case), while plain '<' sorts 'Widgets.ts' first ('W' is a lower UTF-16
  // code unit than 'a'). Reverting the comparator to `localeCompare` turns
  // this assertion red without touching the beta/zeta order at all.
  fixture = createRepoFixture({
    commits: [
      ...
      { date: '2026-07-25T09:00:00+00:00', email: 'ana@example.com', files: ['src/Widgets.ts'] },
      { date: '2026-07-26T09:00:00+00:00', email: 'ana@example.com', files: ['src/Widgets.ts'] },
      { date: '2026-07-27T09:00:00+00:00', email: 'ana@example.com', files: ['src/app.ts'] },
      { date: '2026-07-28T09:00:00+00:00', email: 'ana@example.com', files: ['src/app.ts'] },
    ],
  })

  const heat = await heatTree(fixture.path, '12m', { now: NOW })

  expect(heat.children.map((child) => child.name)).toEqual([
    'Widgets.ts',
    'app.ts',
    'beta.ts',
    'zeta.ts',
